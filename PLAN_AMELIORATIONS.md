...
### 3.1 Nombres Magiques et Constantes

**Problème**: Constantes hardcodées dans le code.

**Exemples**:
```swift
// blocModel.swift: ligne 42-61
let baseDifficulty = pow(1.8, Double(level) - 1)
let normalizedDifficulty = baseDifficulty * 5.0
let styleMultiplier = overhang ? 1.25 : 1.0
let completionBonus = 0.2
// ... etc
```

**Solution**:
```swift
// Créer Models/Constants/ScoringConstants.swift
enum ScoringConstants {
    static let difficultyBase: Double = 1.8
    static let difficultyMultiplier: Double = 5.0
    static let overhangBonus: Double = 1.25
    static let completionBonus: Double = 0.2
    static let efficiencyBonusMax: Double = 0.3
    static let failureBaseScore: Double = 0.3
    static let failureAttemptBonus: Double = 0.05
    static let failureMaxScore: Double = 0.8
    static let maxAttempts: Int = 20
    static let minAttempts: Int = 1
}
...
enum UIConstants {
    ...
    static let chartHeightSmall: CGFloat = 180
    static let chartHeightMedium: CGFloat = 220
}
```

**Tâches**:
1. Extraire toutes les constantes
2. Créer fichiers de constants organisés
3. Documenter signification de chaque constante
4. Remplacer valeurs hardcodées
...
### 3.3 Nommage Incohérent

**Problème**: Conventions de nommage de fichiers inconsistantes.

**Exemples**:
- `sessionModel.swift` (lowercase)
- `BadgesRegistry.swift` (PascalCase)
- `Date+FrenchFormatting.swift` (PascalCase avec +)

**Solution**: Standardiser sur PascalCase pour tous les fichiers.

**Tâches**:
1. Renommer `sessionModel.swift` → `SessionModel.swift`
2. Renommer `blocModel.swift` → `BlocModel.swift`
3. Vérifier cohérence dans tout le projet
...
## 5. Internationalisation (Priorité Basse)

### 5.1 Support Multilingue

**Problème**: Strings hardcodées en français.
...
// Ou avec String catalog (iOS 17+)
Text("Level \(level)", comment: "Bloc difficulty level")
```

**Tâches**:
1. Extraire toutes les strings
2. Créer fichiers .strings ou String Catalog
3. Traduire en anglais
4. Tester changement de langue
5. Localiser formatage dates/nombres
...
### 6.2 Bug: Suppression de Blocs

**Fichier**: `ActiveSessionView.swift`: ligne 183

**Problème**:
```swift
session.blocs.removeAll { $0.id == bloc.id }
```
- N'utilise pas `modelContext.delete()`
- Peut causer données orphelines

**Solution**:
```swift
if let blocToDelete = session.blocs.first(where: { $0.id == bloc.id }) {
    modelContext.delete(blocToDelete)
    try? modelContext.save()
}
```
